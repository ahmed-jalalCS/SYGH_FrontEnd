...
export default function DepartmentManagementPage() {
  ...
  const handleDelete = async (id) => {
    const confirm = window.confirm("هل أنت متأكد من حذف هذا القسم؟");
    if (!confirm) return;
  ...
  return (
    <div className="space-y-6 p-4 animate-fadeIn text-right" dir="rtl">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-bold text-gray-800">🏛 إدارة الأقسام</h2>
        <button
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-100">
            <tr>
              <th className="text-right px-4 py-2">اسم القسم</th>
              <th className="text-center px-4 py-2">إجراءات</th>
            </tr>
          </thead>
          <tbody>
            {departments.map((dept) => (
              ...
              >
                ...
                <td className="px-4 py-2 text-center space-x-3 space-x-reverse">
                  <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
            ...
            <div className="flex justify-end gap-3">
              ...
              >
                <FiCheckCircle /> {mode === "edit" ? "تحديث" : "حفظ"}
              </button>
            </div>
          </div>
        </div>
