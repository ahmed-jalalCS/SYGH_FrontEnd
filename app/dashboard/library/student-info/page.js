...
export default function StudentInfoPage() {
    ...
    try {
      ...
      if (response.data.success && Array.isArray(response.data.data)) {
        ...
        const uniqueDepartments = [
          ...new Set(
            studentData.map((student) => student.department).filter(Boolean)
          ),
        ];
          ...
          ...new Set(
            studentData
              .map((student) => {
                ...
                if (
                  typeof student.graduation_year === "string" &&
                  student.graduation_year.includes("-")
                ) {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", "معلومات_الطلاب.csv");
    link.style.visibility = "hidden";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    toast.success("تم تصدير المعلومات بنجاح");
  };

  const formatGraduationYear = (yearValue) => {
    if (!yearValue) return "غير محددة";
    ...
    return students.filter((student) => {
      ...
      const matchesYear =
        ...
        (student.graduation_year &&
          formatGraduationYear(student.graduation_year) === filters.year);
    ...
    <div className="p-6 max-w-7xl mx-auto">
          ...
          >
            ...
            {Object.values(filters).some((val) => val !== "") ? (
              <span className="flex items-center">
                الفلاتر ({Object.values(filters).filter((f) => f !== "").length}
                )
                <FiX
        ...
        <div className="bg-white p-8 shadow-md rounded-lg text-center text-gray-600">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {search.trim() || Object.values(filters).some((val) => val !== "")
              ? "جرّب تعديل الفلاتر أو معايير البحث"
              : "لا توجد بيانات طلاب متاحة حاليًا."}
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-100 text-right">
                <tr>
                  ...
                  <th className="px-4 py-3 text-gray-600 font-medium">القسم</th>
                  <th className="px-4 py-3 text-gray-600 font-medium">
                    سنة التخرج
                  </th>
                </tr>
                ...
                {filteredStudents.map((student) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-blue-600 flex items-center">
                      ...
                      <button
                        onClick={() =>
                          copyToClipboard(student.email, `email-${student.id}`)
                        }
      ...
      {!loading && filteredStudents.length > 0 && (
        <div className="mt-4 text-sm text-gray-500">
          عرض {filteredStudents.length}{" "}
          {filteredStudents.length === 1 ? "طالب" : "طلاب"}
          {search.trim() && ` نتيجة ل "${search}"`}
          {filters.department && ` في قسم ${filters.department}`}
          {filters.year && ` المتوقع تخرجهم في ${filters.year}`}
        </div>
      )}
    </div>
  );
